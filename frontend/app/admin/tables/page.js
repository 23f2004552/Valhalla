...
export default function TableQRPage() {
  ...
  useEffect(() => {
    // Detect the base URL from the current window location
    if (typeof window !== "undefined") {
      setBaseUrl(`${window.location.origin}/menu`);
    }
  }, []);

  // Generate QR code using a canvas-based approach (no external lib)
  useEffect(() => {
    if (!baseUrl) return;
    ...
    script.onload = () => {
      TABLES.forEach((table) => {
        const canvas = canvasRefs.current[table];
        if (canvas && window.QRCode) {
          window.QRCode.toCanvas(canvas, `${baseUrl}?table=${table}`, {
            width: 200,
            margin: 2,
    ...
    <div className="max-w-[1920px] mx-auto text-admin-text">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {TABLES.map((table) => (
          <div
            key={table}
            className="bg-admin-card border border-white/10 rounded-xl p-6 flex flex-col items-center hover:border-accent-gold/30 transition-all duration-300 group print:break-inside-avoid"
          >
            ...
            <div className="bg-white rounded-lg p-3 mb-4 shadow-lg shadow-black/20">
              <canvas
                ref={(el) => (canvasRefs.current[table] = el)}
            ...
            <button
              ...
              className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full text-xs text-white/60 hover:text-accent-gold hover:border-accent-gold/30 transition-all font-mono tracking-widest uppercase opacity-0 group-hover:opacity-100 print:hidden"
            >
              <LuDownload size={12} />
              Download
            </button>
